...
def plot_geometry(ogeo,angles=np.linspace(0,2*np.pi,100),pos=None,rotation="SD",animate=False,fname=None):
    """
    Plot TIGRE geometry
    ...
    animate : bool, optional
        Plot animated geometry setup or static geometry. The default is False.
...
#--------------------------------------------------------------------------
    ...
    if pos == None or pos < 0 or pos > len(angles):
        pos = 0
        print("Pos should be within the range of [{}, {}], {} is used.".format(0,len(angles),pos))
    ...
    Rs = Rotation.from_euler('ZYZ', geo.angles).as_matrix()  # source and detector rotation are in opposite direction to that of object
    ...
    if rotation in "SD":
        ...
        if np.ptp(stj,axis=0).any():
            ax.plot3D(stj[:,0],stj[:,1],stj[:,2],color='plum',ls='',marker=".",markersize=2.5,mfc='plum',mec="plum")       
    elif rotation in "obj":
        stj[pos,:] = np.matmul(Rs[pos,:,:], scent[pos,:])
        # displacement in y for geo.COR
        if hasattr(geo, 'COR'):
            stj[pos,1] += geo.COR[pos]
        ax.plot3D(stj[pos,0],stj[pos,1],stj[pos,2],color='plum',ls='',marker=".",markersize=2.5,mfc='plum',mec="plum")
        
    # source centre at pos
    source=ax.scatter(stj[pos,0],stj[pos,1],stj[pos,2],color='r',s=5)
    stext=ax.text(stj[pos,0],stj[pos,1],stj[pos,2]+30,'S',None)
    ...
    if rotation in 'SD':
        ...
        if np.ptp(dtj,axis=0).any():
            ax.plot3D(dtj[:,0],dtj[:,1],dtj[:,2],color='skyblue',ls='',marker=".",markersize=2,mfc='skyblue',mec="skyblue")
    elif rotation in 'obj':
        dtj[pos,:] = np.matmul(Rs[pos,:,:], dcent[pos,:]) 
        if hasattr(geo, 'COR'):
            dtj[:,1] += geo.COR
        ax.plot3D(dtj[pos,0],dtj[pos,1],dtj[pos,2],color='skyblue',ls='',marker=".",markersize=2,mfc='skyblue',mec="skyblue")
    # detector centre at pos
    det=ax.scatter(dtj[pos,0],dtj[pos,1],dtj[pos,2],color='brown',s=5)
    dtext=ax.text(dtj[pos,0],dtj[pos,1],dtj[pos,2]-30,'D',None)
    # after scan to pos, then rotate the detector (roll,pitch,yaw) <==> (x,y,z)
    R = np.zeros_like(Rs)
    for j in range(Rs.shape[0]):
        R[j,:,:] = np.matmul( Rs[j,:,:], Rotation.from_euler('XYZ', geo.rotDetector[j,:]).as_matrix())
    ...
    dverts = calCube(dtj,dsz,R,offcent=np.array([-ddp/2,0,0]))
    ...
    if np.ptp(otj,axis=0).any():
        ax.plot3D(otj[:,0],otj[:,1],otj[:,2],color='grey',ls='',marker=".",markersize=2,mfc='grey',mec="grey")        
    # if rotation in "SD":
    #     ax.plot3D(otj[pos,0],otj[pos,1],otj[pos,2],color='grey',ls='',marker=".",markersize=2,mfc='grey',mec="grey") 
    # elif rotation in "obj":
    #     Ro = Rotation.from_euler('ZYZ', -geo.angles).as_matrix()
    #     ax.plot3D(otj[:,0],otj[:,1],otj[:,2],color='grey',ls='',marker=".",markersize=2,mfc='grey',mec="grey")  
    ...
    if rotation in "SD":
        ...
        Ro = Rotation.from_euler('ZYZ', -geo.angles).as_matrix()        
        overts = calCube(otj, geo.sVoxel[[2,1,0]], Ro)         
#    overts = calCube(otj,geo.sVoxel[[2,1,0]])                              
    ...
    def update(pos,stj,dtj,otj,dverts,overts,rotation):
        ...
        if rotation in 'SD':
            ...
            ax.set_title('Current CBCT object at angles {} (deg), in scale'.format(list(map('{:.1f}'.format,-geo.angles[pos]/np.pi*180))) \
                     +'\n\nDetector rotation [{:.1f} - {:.1f}, {:.1f} - {:.1f}, {:.1f} - {:.1f}] (deg)'.format(roll[0],roll[-1],pitch[0],pitch[-1],yaw[0],yaw[-1]))
    

    if animate:
        ani = animation.FuncAnimation(fig, update, len(angles), fargs=(stj,dtj,otj,dverts,overts,rotation), interval=100)
...
def calCube(centre,size,R='eye',offcent=np.array([0,0,0])):
    # Calculate vertices of a cuboid, centred at "centre" with "size" and rotated by R, 3D rotation matrix 
    # centre is either (n,3) or (3,) array and R is (n,3,3) or (3,3) array  
    ...
    if centre.ndim>1:
        ...
        for i in range(n):
            ...
            if isinstance(R, str):
                c = np.matmul( corner*size/2 + offcent, np.eye(3) ) + centre[i,:] 
            else:
                c = np.matmul( corner*size/2 + offcent, R[i,:,:].T ) + centre[i,:]
        ...
        if isinstance(R, str):
            c = np.matmul( corner*size/2 + offcent, np.eye(3) ) + centre
        else:    
            c = np.matmul( corner*size/2 + offcent, R.T ) + centre
...
#    pos = np.pi / 6
    ...
    ani1=plot_geometry(geo, angles, pos, rotation="obj", animate=True, fname="Rotate object")
    ani1
#    plt.show()
    ani2=plot_geometry(geo, angles, pos, rotation="SD", animate=True, fname="Rotate source and detector")
    ani2
    # plt.show()
