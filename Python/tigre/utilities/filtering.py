...
def filtering(proj, geo, angles, parker, verbose=False):
    ...
    with device_manager(proj) as device:
        ...
        # Padding and scale factor
        padding = int((filt_len-geo.nDetector[1])//2 )
        scale_factor = xp.array((geo.DSD[0]/geo.DSO[0]) * (2 * np.pi/ len(angles)) / ( 4 * geo.dDetector[0] ))
        ...
        if angles.shape[0] % 2:
            ...
            fproj=fproj*filt
            fproj=ifft(fproj,axis=1)
            proj[angles.shape[0]-1]=fproj.real[:,padding:padding+geo.nDetector[1]] * scale_factor
