...
# ani1=tigre.plot_geometry(geo,angles,0,animate=True)  # angle=0, S is at (x=DSO, y=0, z=0)
# ani1
...
geo.rotDetector=np.radians( np.array([0, 0, 0]) ) # (roll, pitch, yaw)
...
ani2=tigre.plot_geometry(geo,angles,15,animate=True)
ani2
...
# s_pos = np.vstack([r*np.cos(angles)+soff[0], r*np.sin(angles)+soff[1], 0*angles]).T
# geo21 = tigre.ArbitrarySourceDetMoveGeo(geo,s_pos)
# ani21=tigre.plot_geometry(geo21, geo21.angles, 0, animate=True, fname='Tomosynthesis2')
# ani21
...
# #ani3=tigre.plot_geometry(geo3, geo3.angles, rotation='obj', animate=True)
# ani3=tigre.plot_geometry(geo3, geo3.angles, rotation='SD', animate=True)
# ani3
...
# ## plot 5, fixed target object and detector positions and orientations, source moving linearly, in three orientations, jointly

# geo = tigre.geometry_default()
# geo.DSO = 183
# geo.DSD = 442.5
# df = np.linspace(-256,256,128)  # source position on
# n = len(df)
# R = geo.DSD-geo.DSO
# s_pos = np.zeros((2*n,3))
# d_pos = np.zeros((2*n,3))
# d_rot = np.zeros((2*n,3))
...
# s_pos[:n,2] = df/10
# s_pos[n:2*n,0] = -df
# s_pos[n:2*n,1] = -geo.DSO

# # detector centre positions
# d_pos[:n,0] = -R
# d_pos[n:2*n,1] = R

# # detector rotation (roll,pitch,yaw) away from direction of (0, 0, 0)
# d_rot[:n,0] = 10
# d_rot[n:2*n,0] = -10

# geo5 = tigre.ArbitrarySourceDetMoveGeo(geo, s_pos, d_pos, d_rot)

# ani5=tigre.plot_geometry(geo5, geo5.angles, 0, animate=True, fname='Arbitrary Source')
# ani5
...
# ani6=tigre.plot_geometry(geo6, geo6.angles, 0, animate=True, fname='Spiral Source')
# ani6
